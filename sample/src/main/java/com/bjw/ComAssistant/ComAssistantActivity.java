...
//		}
    ...
    private class DispQueueThread extends Thread{
		...
		public void run() {
			super.run();
			Log.i("xxffff：","while(!isInterrupted()) {");
			while(!isInterrupted()) {
				final ComBean ComData;
		        while((ComData=QueueList.poll())!=null)
		        {
					Log.i("xxffff：","while((ComData=QueueList.poll())!=null)");
		        	runOnUiThread(new Runnable()
					{
						public void run()
						{
							Log.i("xxffff：","runOnUiThread(new Runnable()");
							DispRecData(ComData);
						}
					});
		        	try
    ...
    private void DispRecData(ComBean ComRecData){
		Log.i("xxffff：","DispRecData(ComBean ComRecData){");
